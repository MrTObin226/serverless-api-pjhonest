...
# 1. Системные зависимости (Добавлен curl и build-essential для компиляции sageattn)
...
# 3. Python-зависимости (Разбито на части для стабильности)
RUN python -m pip install -U pip && \
    python -m pip install --no-cache-dir opencv-python-headless accelerate transformers diffusers ftfy Pillow einops imageio-ffmpeg pandas && \
    python -m pip install --no-cache-dir gguf sentencepiece

# Установка sageattn отдельно (она самая капризная)
RUN python -m pip install --no-cache-dir sageattn || echo "⚠️ SageAttn failed, but continuing..."
...
# 5. Модели (Исправлены пути: T5 в text_encoders!)
...
# 6. LoRA (с обманным окончанием для CivitAI)
...
# 7. Копирование конфигов и файлов
