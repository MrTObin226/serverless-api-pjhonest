...
# 3. Скачиваем последнюю стабильную версию ComfyUI
RUN git clone https://github.com/comfyanonymous/ComfyUI.git /ComfyUI && \
    cd /ComfyUI && git checkout master

WORKDIR /ComfyUI

# 4. Установка зависимостей ПОЭТАПНО (ключевое исправление)
# Шаг 1: Обновляем pip и устанавливаем базовые зависимости ComfyUI
RUN pip install --no-cache-dir --upgrade pip setuptools wheel && \
    pip install --no-cache-dir -r requirements.txt

# Шаг 2: Дополнительные зависимости БЕЗ проблемных пакетов (flash-attn/sageattention)
...
# 6. Устанавливаем зависимости нод (с обработкой ошибок)
RUN cd custom_nodes/ComfyUI-WanVideoWrapper && \
    pip install --no-cache-dir -r requirements.txt || echo "Non-critical dependency failed" && \
