...
# 2. Установка системных утилит (Добавлен unzip и замена wget на curl для надежности)
RUN apt-get update && apt-get install -y \
    libgl1 libglib2.0-0 ffmpeg git wget curl unzip libgomp1 ninja-build \
    && rm -rf /var/lib/apt/lists/*

# 3. Скачивание ComfyUI (Используем проверенный метод через curl)
# Мы скачиваем архив и РАСПАКОВЫВАЕМ его сразу в /ComfyUI без промежуточных папок
