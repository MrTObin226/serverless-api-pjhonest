...
# 1. Системные зависимости (минимум для OpenCV + ffmpeg)
...
# 3. Python-зависимости ТОЛЬКО для WanVideo
RUN pip install opencv-python-headless accelerate transformers diffusers ftfy Pillow einops imageio-ffmpeg

# 4. КРИТИЧЕСКАЯ ПРОВЕРКА (падает на этапе сборки!)
RUN python -c "import ftfy; import cv2; print('✅ ftfy и cv2 работают')" || (echo '❌ ОШИБКА' && exit 1)

# 5. ТОЛЬКО WanVideoWrapper + Blockswap (если используете)
RUN cd /ComfyUI/custom_nodes && \
    git clone https://github.com/kijai/ComfyUI-WanVideoWrapper.git && \
    git clone https://github.com/orssorbit/ComfyUI-wanBlockswap.git

# 6. Модели (только HIGH + ваши LoRA)
...
RUN curl -L "https://civitai.com/api/download/models/2553271?type=Model&format=SafeTensor" \
